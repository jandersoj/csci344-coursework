...
def exercise3():
    ...
    if search_term is None:
        return "search term is a required query parameter"
    if location is None:
    ...
    url = f'{base_url}?location={location}&term={location}&limit={count}'
...
def exercise6():
    ...
    if not (location and search_term):
        return '"location" and "term" are required query parameters'
    base_url = 'https://www.apitutor.org/yelp/simple/v3/businesses/search'
    url = f'{base_url}?location={location}&term={search_term}&limit={10}'
