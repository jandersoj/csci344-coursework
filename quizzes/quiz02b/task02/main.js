...
async function getWikipediaArticle(searchTerm) {
  // your code here...
  try {
    response = await fetch(`${ENDPOINT_WIKIPEDIA}/${searchTerm}`);
    const data = await response.json();
    console.log("data got!");
    return data;
  } catch (error) {
    console.error("girl something went wronggg");
  }
}

function dataToHTML(wikiArticle) {
  // your code here...
  console.log("making card for ", wikiArticle);
  const template = `
  <section class="card">
    <img src=${wikiArticle.thumbnail.source}>
    <div>
        <h2>${wikiArticle.title}</h2>
        <p>${wikiArticle.extract_html}</p>
    </div>
    </section>
  `;
